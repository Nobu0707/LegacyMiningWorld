package net.nobu0707.legacyminingworld.geology;

import java.util.Objects;

/** Applies planner output in stable order while enforcing the Paper access boundary. */
public final class LegacyGeologyApplicator {
    private final LegacyGeologyPlanner planner;

    public LegacyGeologyApplicator() {
        this(new LegacyGeologyPlanner());
    }

    LegacyGeologyApplicator(LegacyGeologyPlanner planner) {
        this.planner = Objects.requireNonNull(planner, "planner");
    }

    public ApplicationSummary apply(
            long worldSeed,
            int targetChunkX,
            int targetChunkZ,
            int minimumY,
            int maximumYExclusive,
            LegacyGeologyBlockAccess blockAccess) {
        validateRange(minimumY, maximumYExclusive);
        Objects.requireNonNull(blockAccess, "blockAccess");
        MutableSummary summary = new MutableSummary();
        planner.plan(worldSeed, targetChunkX, targetChunkZ, placement -> applyPlacement(
                placement,
                targetChunkX,
                targetChunkZ,
                minimumY,
                maximumYExclusive,
                blockAccess,
                summary));
        return summary.snapshot();
    }

    ApplicationSummary applyPlacements(
            Iterable<LegacyPlacement> placements,
            int targetChunkX,
            int targetChunkZ,
            int minimumY,
            int maximumYExclusive,
            LegacyGeologyBlockAccess blockAccess) {
        validateRange(minimumY, maximumYExclusive);
        Objects.requireNonNull(placements, "placements");
        Objects.requireNonNull(blockAccess, "blockAccess");
        MutableSummary summary = new MutableSummary();
        for (LegacyPlacement placement : placements) {
            applyPlacement(
                    Objects.requireNonNull(placement, "placement"),
                    targetChunkX,
                    targetChunkZ,
                    minimumY,
                    maximumYExclusive,
                    blockAccess,
                    summary);
        }
        return summary.snapshot();
    }

    private static void applyPlacement(
            LegacyPlacement placement,
            int targetChunkX,
            int targetChunkZ,
            int minimumY,
            int maximumYExclusive,
            LegacyGeologyBlockAccess blockAccess,
            MutableSummary summary) {
        summary.planned++;
        if (Math.floorDiv(placement.x(), LegacyGeologySettings.CHUNK_SIZE) != targetChunkX
                || Math.floorDiv(placement.z(), LegacyGeologySettings.CHUNK_SIZE) != targetChunkZ) {
            summary.skippedOutsideTargetChunk++;
            return;
        }
        if (placement.y() < minimumY || placement.y() >= maximumYExclusive) {
            summary.skippedByHeight++;
            return;
        }
        if (!blockAccess.isAccessible(placement.x(), placement.y(), placement.z())) {
            summary.skippedOutOfRegion++;
            return;
        }
        LegacyBlockKind current = blockAccess.getBlockKind(
                placement.x(), placement.y(), placement.z());
        if (!LegacyReplaceableBlock.isReplaceable(current)) {
            summary.skippedNotReplaceable++;
            return;
        }
        blockAccess.setMaterial(
                placement.x(), placement.y(), placement.z(), placement.material());
        summary.applied++;
    }

    private static void validateRange(int minimumY, int maximumYExclusive) {
        if (minimumY >= maximumYExclusive) {
            throw new IllegalArgumentException("minimumY must be less than maximumYExclusive");
        }
    }

    public record ApplicationSummary(
            int planned,
            int applied,
            int skippedByHeight,
            int skippedOutsideTargetChunk,
            int skippedOutOfRegion,
            int skippedNotReplaceable) {
    }

    private static final class MutableSummary {
        private int planned;
        private int applied;
        private int skippedByHeight;
        private int skippedOutsideTargetChunk;
        private int skippedOutOfRegion;
        private int skippedNotReplaceable;

        private ApplicationSummary snapshot() {
            return new ApplicationSummary(
                    planned,
                    applied,
                    skippedByHeight,
                    skippedOutsideTargetChunk,
                    skippedOutOfRegion,
                    skippedNotReplaceable);
        }
    }
}
