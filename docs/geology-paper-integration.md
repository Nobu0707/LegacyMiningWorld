...
Multiverse-Core統合・大量chunk性能試験・禁止block完全走査は後続のPhase 4A/4B1で完了した。production依存、plugin descriptor、通常Paper smoke、release JARにMultiverse-Coreを含めない境界は維持した。

Phase 5/version `1.0.0`では、このPaper統合、地質設定、seed式、replacement、10 anchors、geology-only goldenを`1.0.0-rc.1`から変更していない。RC/stableのproduction class payloadは同一で、stable JAR、package JAR、clean-roomの回帰はPASSした。最終結果は[stable release情報](stable-release.md)を参照する。
