...
Phase 4Aでは`src/multiverseVerifier`を独立source setとし、当時の`LegacyMiningWorld-MultiverseVerifier-0.5.0-alpha.1.jar`を`multiverseVerifierJar` taskで作成した。plugin名は`LegacyMiningWorldMultiverseVerifier`、hard dependはtest JAR内だけの`LegacyMiningWorld`と`Multiverse-Core`。Paper APIだけでcompileし、Multiverse API、NMS、CraftBukkit内部、reflection、network、scheduler、block write、world削除を使わない。
...
## Phase 4B1/4B2で完了した拡張

Phase 4Aは固定4チャンクの全高完全走査と同一保存worldの再読込までだった。Phase 4B1/version 0.6.0-alpha.1で1,089chunk、MCA header、distribution、performance、別clean worldのforward/reverse再生成比較を完了した。Phase 4B2/version 1.0.0-rc.1では同じ検証をpackage JARとclean-roomで回帰し、production Multiverse依存0とtest-only verifierの非配布を最終監査した。詳細は`docs/large-scale-validation.md`を参照する。

## Phase 5 stable回帰

version `1.0.0`では、production class payloadを`1.0.0-rc.1`から変更せず、stable production JARとtest-only `LegacyMiningWorld-MultiverseVerifier-1.0.0.jar`を使ってgenerator列挙、world create、保存、restart、auto-load、全高scanを再実行した。seed、generator、spawn、anchors、Y=5..67 count/checksum、forbidden 0、biome結果は維持した。

verifier JARはstableでもserver導入・release package収録の対象ではない。production pluginのMultiverse API依存、`depend`、`softdepend`は引き続き0である。成果物と最終matrixは[stable release情報](stable-release.md)を参照する。
