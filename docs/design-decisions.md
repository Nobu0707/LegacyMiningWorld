...
- Paper 26のcustom world保存先はworld root直下ではなくdimension folderであるため、filesystem検査は検証worldの`world/dimensions/minecraft/legacy_mining_mv_smoke/`とPaper UUID metadataを対象にする。
...
- versionを`1.0.0-rc.1`とし、機能追加は行わない。production JavaはPhase 4B1 baseline `71b5deb151041f5c9e85a84447a454dbb5ab68a4`との完全一致を監査する。
- 全JavaCompileへ`-Xlint:all -Werror`を適用し、production/test-only双方のwarningをbuild failureにする。依存関係はGradleの解決結果と宣言を別々に監査する。
- Gradle JAR taskはtimestampを保存せずentry順を再現可能にし、production/verifierを2回の独立clean buildでbyte比較する。
- release tar.gzは固定mtime、uid/gid 0、filename sort、`gzip -n`を使う。production JAR、README、release notes、canonical SHA256SUMS、ライセンス未選択markerだけを含む。
- `LICENSE-NOT-SELECTED.txt`はライセンス付与ではなく、再配布条件未定を明示する。ライセンス文面を推測しない。
- clean-roomはcommit済みtracked sourceとbuild outputを分離するdetached worktreeを使う。既存Gradle cacheは再利用するが、main側のbuild/world/logは持ち込まず、Paper/EULA/Multiverseだけを明示コピーする。
- `multiverseVerifierTest`は4chunk検証、`largeScaleVerifierTest`は大規模verifierのtagへ分離し、同じtest taskを二重実行しない。
- verifierのunload false件数はgenerator失敗ではなくPaper ticket lifecycleの観測なので`immediateUnloadRejected`と表現する。
- 通常Paper smokeにはrelease packageから展開してSHA256SUMS検証済みのJARを使い、`build/libs`のJARとbyte一致を必須にする。
- test-only verifier、anchors、spec、server、runtime world、review logは配布物から除外する。
- cleanなtracked source、再現可能JAR/package、clean-room結果を同一HEAD/versionで結び付ける。archive連続実行ではこの条件を満たす重い結果だけを再利用する。
- Phase 4B2時点ではstable昇格を後続Phaseへ分離し、tag、push、publishを行わない判断とした。

## Phase 5で確定したstable promotion

- stable promotionはversion、文書、test、監査script、package metadataだけを変更し、production機能を変更しない。
- production Javaとproduction `plugin.yml` templateはRC baseline `3c30291b5c570d1c53a261ef8f5d9715b42512ff`から変更しない。Paper依存、feature設定、seed、salt、anchors、large-scale specも同一に保つ。
- RCとstableを別worktreeでbuildし、production class entry名と各class bytesを比較する。functional class payloadは同一でなければstableへ昇格しない。
- sourceのproduction `plugin.yml` templateは同一に保ち、JARへ展開された`plugin.yml`だけversion `1.0.0-rc.1`から`1.0.0`へ変える。他のmetadata差は許可しない。
- production欠陥が見つかった場合、その修正をstable promotionへ混在させない。昇格を中止し、必要なら`1.0.0-rc.2`等の別RCで扱う。
- ライセンス未選択でもtechnical stableとするが、成果物はprivate/internal deploymentだけを対象とする。stableとpublicly releasedを同義にしない。
- Git tag、push、GitHub Release、Maven publish、外部uploadをPhase 5で行わない。
- 人間向けユーザー受入チェックリストを作成するが、Codexは手動プレイ結果を実施済みとして記入しない。長時間運用も自動検証済みとは主張しない。
- public releaseはoptional Phase 6へ分離し、ライセンス、配布条件、tag/release/signing/external publicationにはユーザーの明示承認を要求する。
- `1.0.0-rc.1`へのrollbackはserver停止・backup・JAR 1本の置換で行える。RC/stable間にconfig、database、runtime data migrationはなく、production class payloadも同一である。

## Phase 6で確定したMIT/public-ready patch

- versionを`1.0.1`とし、MIT License（SPDX: `MIT`、Copyright (c) 2026 nobu0707）を採用する。正本はroot `LICENSE`とする。
- `src/main/java/**`、production `plugin.yml`テンプレート、Paper dependency、generator/seed/salt/placement、anchors、large-scale specはbaseline `2c487560b0d862df0af0c452c3686a7ca72fade3`から変更しない。
- production/verifier JARの`META-INF/LICENSE`とrelease packageの`LICENSE`はroot LICENSEとbyte一致させる。Java source個別headerは追加しない。
- `1.0.0`成果物を差し替えず、package内容と配布条件の変更をpatch version `1.0.1`として公開可能にする。
- `1.0.0`と`1.0.1`を別worktreeでtest除外buildし、production class entry名と各class SHA-256が同一、`plugin.yml`がversion-onlyであることを検査する。
- production Javaと生成ロジックが不変なので、unit/integration test、Paper/Multiverse smoke、large-scale、clean-room全回帰をPhase 6では再実行しない。過去ログを1.0.1の実行結果として扱わない。
- config/data migrationとworld再作成は不要で、既存chunkへのretro-generationは引き続き行わない。
- packageはproduction JAR、README、release notes、SHA256SUMS、LICENSEだけとし、固定metadataで再現可能にする。
- public-distribution-readyとexternal publicationを分離する。Phase 6ではtag、push、release作成、publish、uploadを行わない。
